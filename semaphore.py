"""
Функционал семафор реализуется неявно в основном файле - внутри ThreadPoolExecutor. Здесь попытаюсь заменить его на явную реализацию.
...
class Factorization:
    def __init__(self, input_filename, output_filename, num_threads):
        ...
        self.semaphore = threading.Semaphore(self.num_threads)
        self.locker = threading.Lock() # чтобы записи в нескольких потоках не пересекались
    ...
    def semaphore_worker(self, num, file_name):
        ...
        try:
            ...
            with self.locker:
                res_str = self.process_number(num)
        ...
        with open(self.input_filename, 'r') as f, open(self.output_filename, 'w') as w:
            for line in f:
                ...
                for num_str in nums:
                    ...
                    t = threading.Thread(target=self.semaphore_worker, args=(num, w))
                    threads.append(t)
                    t.start()
